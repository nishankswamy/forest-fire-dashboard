...
Quantified (`tools/energy.py`), the radio accounts for **1.2% of node power**
at the busiest node and **0.4% at a plain member**. Disabling radio sleep
entirely — discarding the central energy mechanism of this project — changes
projected battery life by about 4%. The first-order model, applied unmodified,
overstates battery life by roughly **1450×**.
...
**Reproducibility.** Every figure in this section is regenerated by a single
committed script:

```bash
python3 pi/tools/experiments.py --md      # all experiments, markdown tables
python3 pi/tools/experiments.py --csv out/
```

Results produced by scripts that are not in the repository are assertions
rather than evidence. `experiments.py` runs the deployment firmware and the
routing layer directly, so any claim below can be checked by running it and
comparing. Two further tools support the open questions: `tools/energy.py`
computes the whole-node power budget (RQ7), and `tools/fieldreport.py` produces
the §7.10 measurement table from live gateway data (RQ6).
...
### 7.11 Regulatory Airtime *(simulated)*

Licence-exempt ISM bands cap transmitter duty cycle. The relevant limit is
per **device**, not per channel, so the busiest node governs.

| Node | Transmissions per frame | Airtime per hour | Duty |
|---|---|---|---|
| CH-A (1) | 5 | 23.0 s | **0.639%** |
| Node 2 | 1 | 4.6 s | 0.128% |
| Node 3 | 1 | 4.6 s | 0.128% |
| CH-B (4) | 4 | 18.4 s | 0.511% |
| Node 5 | 1 | 4.6 s | 0.128% |
| **Channel total** | | 55.2 s | 1.533% |

CH-A sits at 0.639% — inside a 1% per-device limit, but only by a factor of
1.6. That headroom is not comfortable, and it degrades in two directions:
linearly with the number of members a head serves, and inversely with
`FRAME_SECONDS`. A head serving eight members on a 20-second frame would
exceed 1%.

This is a scaling constraint the design does not currently document, and it
binds earlier than the energy constraint does. Regenerate with
`experiments.py --only airtime` and verify against the regulation applicable
to the deployment region before increasing either cluster size or reporting
rate.
